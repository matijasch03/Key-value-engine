...
/*
func main() {
	bloomFilter := NewBloomFilter(100, 2)
	...
	loadedBloomFilter, err := LoadFromFile("data.gob")
    if err != nil {
        log.Fatal(err)
    }
    ...
    if loadedBloomFilter.Contains(testElement) {
        fmt.Printf("%s može biti u skupu (False positive je moguć)\n", testElement)
    } else {
        fmt.Printf("%s nije u skupu (Sigurno negativan rezultat)\n", testElement)
    }
}
*/